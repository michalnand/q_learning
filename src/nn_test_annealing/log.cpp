...
{
	this->file_name = (char*)malloc(1024*sizeof(char));
...
void CLog::save()
{
	...
	if (log_data.size() > 1)
	{
		for (i = 0; i < (log_data.size()-1); i++)
		{
			if (log_data[i].size() != log_data[i+1].size())
			{
				printf("inconsistent log data size on %u\n", i);
				return;
			}
		}
	}
