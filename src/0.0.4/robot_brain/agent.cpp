...
void CAgent::reset(struct sAgent *agent_)
{
	...
	// q_learning->process(agent.state, agent.reward, 2.0, 0.0);
...
void CAgent::print(std::vector<float> subspace)
{
	...
	CLog log_q_max((char*)"q_max.txt", 3);
	...
	q_learning->save("q_learning.txt");
}
