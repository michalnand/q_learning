...
CBasisFunctions::CBasisFunctions(u32 count, u32 dimension, float a_range, float b_range, float w_range, u32 function_type)
{
  ...
  h = (float*)malloc(this->functions_count*sizeof(float));
  ...
  for (j = 0; j < this->functions_count; j++)
  {
    ...
    if (function_type == BASIS_FUNCTION_TYPE_GAUSS_PEAK)
    {
      float k = 0.90;
      b[j] = this->b_range*(k + (1.0 - k)*abs_(rnd_()));
    }
...
void CBasisFunctions::process_linear_combination(std::vector<float> input)
{
  ...
  linear_combination = 0.0;
  for (j = 0; j < this->functions_count; j++)
    linear_combination+= w[j]*output[j] + output_h[j];
}

void CBasisFunctions::learn_linear_combination(float required_value, float learning_rate, bool learn_basis, float reward_value)
{
  ...
  if (function_type == BASIS_FUNCTION_TYPE_GAUSS_PEAK)
  {
    ...
    float lc = 10.0*learning_rate*tanh(4.0*abs_(required_value));

    for (i = 0; i < this->dimension; i++)
      a[dist_min_idx][i] = (1.0 - lc)*a[dist_min_idx][i] + lc*input[i];


      h[dist_min_idx]+= 0.1*learning_rate*error;

      if (h[dist_min_idx] < -1.0)
        h[dist_min_idx] = -1.0;
