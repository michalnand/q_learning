...
CEchoStateNeuralNetwork::CEchoStateNeuralNetwork(struct sEchoStateNeuralNetworkInitStructure nn_init_structure)
{
	u32 i, j, k;
		...
		for (j = 0; j < nn.size_output[k]; j++)
		{
			...
			for (i = 0; i < nn.size_input_[k]; i++)
			{
				if (k != (nn.layers_count-1))
					nn.w[k][j][i] = nn.weight_range*rnd();
				else
...
CEchoStateNeuralNetwork::~CEchoStateNeuralNetwork()
{
	u32 j, k;
	output.clear();
...
void CEchoStateNeuralNetwork::process(std::vector<float> input)
{
	u32 i, j, k;
...
std::vector<float> CEchoStateNeuralNetwork::get()
{
	return output;
}

void CEchoStateNeuralNetwork::learn(std::vector<float> required_output)
{
	u32 i, j, k, k_;
...
float*** CEchoStateNeuralNetwork::get_weights()
{
	return nn.w;
}

float*** CEchoStateNeuralNetwork::get_dweights()
{
	return nn.dw;
}


void CEchoStateNeuralNetwork::set_weights(float ***w)
{
	merge_weights(w, 0.0);
}

void CEchoStateNeuralNetwork::merge_weights(float ***w, float weight)
{
	if (weight < 0.0)
		weight = 0.0;
...
void CEchoStateNeuralNetwork::merge_dweights(float ***dw)
{
	u32 i, j, k;
	for (k = 0; k < nn.layers_count; k++)
		for (j = 0; j < nn.size_output[k]; j++)
...
float CEchoStateNeuralNetwork::rnd()
{
	return ((rand()%2000000) - 1000000)/1000000.0;
}


void CEchoStateNeuralNetwork::set_learning_constant(float learning_constant)
{
...
i32 CEchoStateNeuralNetwork::save_weights(char *file_name)
{
	u32 i, j, k;
...
i32 CEchoStateNeuralNetwork::load_weights(char *file_name)
{
	u32 i, j, k;
