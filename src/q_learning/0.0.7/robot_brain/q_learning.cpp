                        ...
                        u32 learn, u32 force_learn)
{
  ...
  switch (q_init.function_type)
  {
    ...
    case 1:
            for (j = 0; j < q_init.actions_count; j++)
              actions_score[j] = q_func_sparse->get(q_res.state, actions[j]);
            break;

    case 2:
            for (j = 0; j < q_init.actions_count; j++)
              actions_score[j] = q_func_nn_bfnn_linear->get(q_res.state, actions[j]);
            break;

    case 3:
            for (j = 0; j < q_init.actions_count; j++)
                actions_score[j] = q_func_nn_bfnn_tanh->get(q_res.state, actions[j]);
            break;

    case 4:
            for (j = 0; j < q_init.actions_count; j++)
                actions_score[j] = q_func_nn_bfnn_intersynaptic->get(q_res.state, actions[j]);
            break;
  }
