...
Pre zjednodušený fyzikálny model je robota možné aproximovať kvádrom o rozmeroch $w$, $h$, $d$ a rozhode kolies
...
kde $k$ je konštanta úmernosti a $M_f(t)$ je výsledný točivý moment robota.
...
%Čo po diskretizácií a použitím Z-transformácie a diskretizačným krokom $dt$ vedie na diskrétny prenos v tvare
%\begin{align}
%  \frac{\phi'(z)}{M_f'(z)} = \frac{dt}{J} \frac{1}{ (1 + \frac{b}{J}) + (-2 - \frac{b}{J})z^{-1} + z^{-2} }
%  \label{eq:robot_model_eq_phi_disc}
%\end{align}
%Triviálny prenos regulátora potom bude
%\begin{align}
% C_d(z) = \frac{\frac{J}{dt} ((1 + \frac{b}{J})z^{-1} + (-2 - \frac{b}{J})z^{-2} + z^{-3} )}{1 - z^{-1}}
%\end{align}
%čo je diskrétna forma PD regulátora.
...
%čo vedie na diskrétny ekvivalent PI regulátora
%\begin{align}
% P_v(z) = \frac{ \frac{m}{dt}((1+c)z^{-1} - z^{-2}) }{1-z^{-1}}
%\end{align}
...
$v(n)$ je dopredná rýchlosť a $d(n)$ je rozdiel rýchlosti motorov.
